...
def test_rate_limit_sets_cooldown_and_serves_stale(monkeypatch: pytest.MonkeyPatch) -> None:
    yahoo.reset_yahoo_cache_for_tests()
    yahoo._cache.values = {"global_dow_jones_chg": -0.1}
    yahoo._cache.fetched_at = 500.0
    ...
    result = yahoo.fetch_yahoo_changes({"global_dow_jones_chg": "^DJI"}, now=ts, force=True)
    assert result == {"global_dow_jones_chg": -0.1}
    assert yahoo._cache.cooldown_until > ts
